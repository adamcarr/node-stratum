  ...
  var Server = classes.Base.$define('Server', {
    ...
    getStratumCommands: function (buffer){
      ...
      if (Buffer.isBuffer(buffer) && buffer.length) {
        ...
        console.log('getStratumCommands.string', string);

        // if there was a previous partial command, prepend to string before processing
        // also, clear out partial command for next iteration
        if (self.partialCommand && self.partialCommand.length > 0) {
          console.log('getStratumCommands.prependPartialCommand', self.partialCommand);

          string = self.partialCommand + string;
          self.partialCommand = undefined;
        }

        // we can get partial commands here as the pool may flush the buffer without a complete command.
        // We should split on new line
        // We should then assume if the last item in the array of split commands is not an empty string
        // meaning, it wasn't a terminated command on the socket, we should pool cache this partial command
        // and prepend it to the next buffer output.
        splits = string.split('\n');

        for (var i = 0; i < splits.length; i++) {
          item = splits[i];
          if (!_.isEmpty(item) && !_.isNull(item)) {
            // if it is the last item in the splits array, assume it is an incomplete command and cache for next iteration.
            if (i === splits.length - 1) {
              self.partialCommand = item;
              console.log('getStratumCommands.partialCommand', item);
            } else {
              cmds.push(item);
            }
          }
